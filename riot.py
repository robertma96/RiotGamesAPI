...
import requests
import time
api_key = "RGAPI-34620c42-4420-40b5-93d3-10ad56e8070e"
region = 'EUNE'
...
def getIDMatches():
    ...
    while len(lista_match_id) == 100:
        beginIndex = endIndex
        endIndex = endIndex + 100
        link = "https://" + f(region) + ".api.riotgames.com/lol/match/v3/matchlists/by-account/" + accountID + "?beginIndex=" + str(beginIndex) + "&endIndex=" + str(endIndex) + "&queue=420&season=11&api_key=" + api_key
        response_link = requests.get(link)
        if response_link.status_code == 200:
            obiect = json.loads(response_link.content.decode('utf-8'))
            for i in range(len(obiect['matches'])):
...
def championwinrate():
    ...
    if update == True:
        ...
        for match in meciuri_de_adaugat:
            ...
            k = 0
            WON = None
            link_match = "https://" + f(region) + ".api.riotgames.com/lol/match/v3/matches/" + str(match) + "?api_key=" \
                         + api_key
